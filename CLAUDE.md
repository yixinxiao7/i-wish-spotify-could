...
**i-wish-spotify-could** — A personal Spotify utility web app for categorizing liked songs that aren't in any playlist. Users log in via Spotify OAuth, browse uncategorized songs with in-app playback, and assign them to playlists. It also lets users clean stale songs out of a playlist and propagate songs from one owned playlist into another.
...
- `app/` — pages: `/` (landing), `/login`, `/callback`, `/organize`, `/clean`, `/clean/[playlistId]`, `/propagate`, `/propagate/[destinationId]/from/[sourceId]`
...
- `components/ui/` — SongCard, PlaylistList, PlaylistChooser (shared playlist-picking screen — heading, description, loading/error/empty states, `PlaylistList`), SongListPagination + SongCardSkeleton (shared by every paged song list), TypingHeadline (the rotating, typed-and-erased headline shared by the landing and login pages), + shadcn/ui components
- `hooks/use-deferred-row-action.ts` — the undoable-window machinery (timers, pause/resume on hover/focus, undo, flush on unmount/`pagehide`) shared by every row-level action that defers behind a toast
- `types/spotify.d.ts` — TypeScript interfaces (Song, Playlist, PlaylistSong)
- `utils/config.ts` — API endpoint constants + OAuth scopes
- `utils/pagination.ts` — pure offset/page-clamping and numbered-page-list helpers shared by every paged song list
...
| POST | `/api/playlists/add-song` | Add song to playlists, remove from uncategorized-songs cache, invalidate each target playlist's own song-list cache |
| GET | `/api/playlists/pins` | Pinned playlist IDs |
| POST | `/api/playlists/pin` | Pin or unpin a playlist |
| GET | `/api/playlists/{playlist_id}/songs` | One page of a playlist's songs (`offset`/`limit`/`sort`), plus its total and the listening-affinity availability block. Optional `exclude_playlist_id` (an owned playlist) omits songs that playlist already contains, before ordering and pagination — used by song propagation |
...
- `playlist_songs_service.py` fetches a playlist's full song list (id, name, artists, album, art, `added_at`) with an in-memory per-playlist cache (5-minute freshness, no runtime file), and orders it by `playlist`/`added_asc`/`added_desc`/`affinity_asc` before paginating — ordering is computed over the whole playlist, not just the returned page. `affinity_asc` sorts ascending by tier, breaking ties by oldest `added_at`. `get_playlist_song_ids()` reads the same cache to return just a playlist's song IDs (song propagation's exclusion set), so a destination playlist's contents are fetched once per freshness window rather than once per page turn. `get_playlist_songs_page()`'s optional `exclude_song_ids` is applied to the full song list before ordering and pagination, so the reported `total` reflects the exclusion.
- Song propagation (frontend `/propagate` → `/propagate/[destinationId]/from/[sourceId]`, backend `exclude_playlist_id`): lets a user copy songs from one owned playlist (source) into another (destination). The destination chooser reuses `PlaylistChooser`; picking a destination opens a dialog (the shared `PlaylistList` with `onSelectPlaylist`) listing every other owned playlist as a source. The working page lists the source's songs with the destination's contents excluded server-side, using the same `SongCard`, sort control, and pagination as `/clean/[playlistId]`, but with `SongCard`'s `onAdd` prop (a plus control) instead of `onRemove`, and `showAddToPlaylists={false}` since the destination is already chosen. Adding is deferred through the shared `useDeferredRowAction` hook exactly like cleanup's removal. Because the exclusion-applied `total` reads 0 both when the source is genuinely empty and when every song is already in the destination, the frontend distinguishes them with one extra lightweight request (no `exclude_playlist_id`, `limit=1`) only when the first `total` is 0.
...
- `PlaylistsProvider` (frontend) owns the playlist list and pin toggling for `/organize`, `/clean/[playlistId]` (via `SongCard`'s "add to playlists" dialog), and the `/propagate` chooser, so every rendered `SongCard`'s dialog and both playlist choosers share one pinned state and reorder together. Also exposes `refetch()` for a "try again" action after a failed load. The `/propagate/[destinationId]/from/[sourceId]` working page does not use it — `SongCard`'s add-to-playlists dialog is hidden there (`showAddToPlaylists={false}`), so there is nothing for it to feed.
- Any row-level action deferred behind an undoable window (playlist-cleanup's removal, song-propagation's add) goes through the shared `useDeferredRowAction` hook (`ui/src/hooks/use-deferred-row-action.ts`): the row is hidden immediately and a 10s timer is started; the caller's `perform(row)` fires only when it elapses. Undo clears the timer so `perform` is never called. The undo affordance is a toast action with a depleting progress bar (`ToastProvider`'s `durationMs`/`action`/`progress` options) whose duration matches the timer exactly. Both clocks pause together while the user hovers or keeps focus anywhere inside the toast (`ToastOptions.onPauseChange`, wired to the hook's own per-row timer), so the window can't run out while someone is mid-reach for Undo — a WCAG 2.2.1 requirement, not just a nicety. A pending action is flushed (via `perform(row, {keepalive: true})`) on `pagehide` and on unmount, so navigating away or closing the tab carries it out rather than dropping it — a paused timer is still flushed this way, since the request hasn't been cancelled, only delayed. Each caller (`/clean/[playlistId]`'s `DELETE`, propagation's `POST /add-song`) supplies its own `perform`/`buildToastMessage`/`onError`.
- `TypingHeadline` (`ui/src/components/ui/typing-headline.tsx`) drives the animated headline on **both** entry points, typing each phrase in and erasing it character-by-character behind a blinking caret before the next begins, looping indefinitely. A single `setTimeout`-driven state machine (`typing` → `holding` → `erasing`) schedules exactly one step per commit rather than running overlapping intervals. It pauses on hover or focus (two independent flags, since the pointer can leave while focus remains) and resumes from the same character — the WCAG 2.2.2 mechanism for auto-updating content. Under `prefers-reduced-motion: reduce` (resolved in a mount effect, not at module scope, so SSR and first client paint agree) it renders the first phrase whole with no caret and schedules nothing. The `<h1>`'s `aria-label` always holds the complete phrase; the animating text and caret are `aria-hidden`, and there is no `aria-live` region, so assistive technology never hears a partial phrase or a rotation announcement.
- Two surfaces, two shapes, one component: the **landing page** (`/`) replaces the whole line, cycling `LANDING_HEADLINE_PHRASES` (one clause per tool). The **login page** (`/login`) instead holds a fixed sentence and cycles only its verb, via the optional `prefix`/`suffix` props — `prefix="better "`, `suffix=" your songs"`, `phrases={LOGIN_HEADLINE_VERBS}` (`organize`/`clean`/`propagate`). With a suffix present the caret renders *between* the cycling word and the static suffix, marking the typing position rather than the end of the line, and the `aria-label` is composed as `prefix + phrase + suffix` so the heading always names the whole sentence.
- Both headings reserve a responsive `min-height` sized to their **tallest wrapped state**, so rotation never shifts the content below. These values are measured, not guessed — note the tallest state is not always the longest phrase (mid-type wrapping can peak earlier), and the narrow breakpoint often needs more lines than `sm`+ (login wraps to 3 lines on `propagate` but 2 at `sm`+). Re-measure if the copy, the type scale, or the container width changes.
